...
public class OvfExporter
{
	...
	private static void getConnectionParameters(String[] args) throws IllegalArgumentException
	{
		int ai = 0;
		String param = "";
		String val = "";
	...
	private static void getInputParameters(String[] args)
	{
		...
		while (ai < args.length)
		{
			...
			{
				host = val;
			} else if (param.equalsIgnoreCase("--vapp") && !val.startsWith("--") && !val.isEmpty())
			{
				vApp = val;
		...
		{
			throw new IllegalArgumentException("Expected --host or --vapp arguments or --localPath");
		}
	}
	...
	private static List<DynamicProperty> getDynamicProarray(ManagedObjectReference ref, String type, String propertyString) throws Exception
	{
		PropertySpec propertySpec = new PropertySpec();
		propertySpec.setAll(Boolean.FALSE);
	...
	private static void printHttpNfcLeaseInfo(HttpNfcLeaseInfo info, String hostString)
	{
		...
		if (deviceUrlArr != null)
		{
			...
			for (HttpNfcLeaseDeviceUrl durl : deviceUrlArr)
			{
				...
				System.out.println("   Updated device URL: " + durl.getUrl().replace("*", hostString));
		...
		try
		{
			...
			else
			{
				...
				if (vAppMoRef != null)
				{
					...
					System.out.println("Getting the HTTP NFCLEASE for the vApp: " + vApp);
					...
					{
						List<DynamicProperty> dynaProList = getDynamicProarray(httpNfcLease, "HttpNfcLease", "info");
						HttpNfcLeaseInfo httpNfcLeaseInfo = (HttpNfcLeaseInfo) dynaProList.get(0).getVal();
						...
						leaseExtender = new OvfExporter().new HttpNfcLeaseExtender(httpNfcLease, vimPort);
						...
						if (deviceUrlArr != null)
						{
							...
							for (int i = 0; i < deviceUrlArr.size(); i++)
							{
								...
								long writtenSize = writeVMDKFile(absoluteFile, deviceUrlStr.replace("*", clearIp));
							...
							OvfCreateDescriptorResult ovfCreateDescriptorResult = vimPort.createDescriptor(serviceContent.getOvfManager(), vAppMoRef, ovfCreateDescriptorParams);
							System.out.println();
							...
							out.close();
							System.out.println("OVF Desriptor Written to file " + vApp + ".ovf");
