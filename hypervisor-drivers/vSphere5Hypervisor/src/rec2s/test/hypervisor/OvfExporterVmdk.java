...
public class OvfExporterVmdk
{

	private static class TrustAllTrustManager1 implements javax.net.ssl.TrustManager, javax.net.ssl.X509TrustManager
	{
	...
	private static void getInputParameters(String[] args)
	{
		...
		{
			throw new IllegalArgumentException("Expected --vmname, --host, --localPath arguments properly");
		}
	}
	...
	private static void exportVM() throws Exception
	{
		...
		if (srcMOR == null)
		{
			throw new IllegalArgumentException(" Source Host " + host + " Not Found.");
		}
		ManagedObjectReference vmMoRef = getDecendentMoRef(srcMOR, "VirtualMachine", vmname);
		if (vmMoRef == null)
		{
		...
		try
		{
			...
			if (result[0].equals(HttpNfcLeaseState.READY))
			{
				...
				HttpNfcLeaseInfo httpNfcLeaseInfo = (HttpNfcLeaseInfo) getManagedObjectProperty(httpNfcLease, "info");
				...
				leaseExtender = new OvfExporterVmdk().new HttpNfcLeaseExtender(httpNfcLease, vimPort);
				...
				for (HttpNfcLeaseDeviceUrl deviceUrl : deviceUrlArr)
				{
					...
					System.out.println("VMDK URL: " + deviceUrlStr.replace("*", clearIp));
					long size = writeVMDKFile(absoluteFile, deviceUrlStr.replace("*", clearIp), diskCapacity, vmname);
					//TODO create OVF file for the vmdk
					OvfFileCreator ofc = new OvfFileCreator();
					ofc.setOvfFileName(localPath + "/" + /*vmname*/ "template" + ".ovf");
					ofc.setHardDiskImage(vmname + "-" + absoluteFile);
					ofc.setHardDiskSize(size+"");
					ofc.generateXML();
					break;
				}
	...
	public static Object getManagedObjectProperty(ManagedObjectReference mor, String property)
	{
		Object retVal = null;
		try
		{
			...
			List<PropertyFilterSpec> listpfs = new ArrayList<PropertyFilterSpec>(1);
			listpfs.add(propertyFilterSpec);
			List<ObjectContent> oCont = vimPort.retrieveProperties(propCollectorRef, listpfs);
			if (oCont != null)
			{
	...
	private static void printHttpNfcLeaseInfo(HttpNfcLeaseInfo info, String hostName)
	{
		...
		for (HttpNfcLeaseDeviceUrl durl : deviceUrlArr)
		{
			...
			System.out.println("   Updated device URL: " + durl.getUrl().replace("*", hostName));
	...
	public class HttpNfcLeaseExtender implements Runnable
	{
		...
		public HttpNfcLeaseExtender(ManagedObjectReference mor, VimPortType VIM_PORT)
		{
			httpNfcLease = mor;
			vimPort = VIM_PORT;
		}
		...
		public void run()
		{
			try
			{
				System.out.println("---------------------- Thread for " + "Checking the HTTP NFCLEASE vmdkFlag: " + vmdkFlag + "----------------------");
				while (!vmdkFlag)
				{
	...
	private static long writeVMDKFile(String absoluteFile, String string, long diskCapacity, String vmName) throws Exception
	{
		...
		System.out.println("Exported File " + vmName + "-" + absoluteFile + " : " + written);
		//testing1323778883554-disk-0.vmdk
		in.close();
		out.close();
		return written;
	}
	 ...
	 */
	private static ManagedObjectReference getDecendentMoRef(ManagedObjectReference root, String type, String name) throws Exception
	{
	 ...
	 */
	private static List<ObjectContent> getContentsRecursively(ManagedObjectReference collector, ManagedObjectReference root, String[][] typeinfo, boolean recurse) throws Exception
	{
	...
	private static void printUsage()
	{
		...
		System.out.println("--url [webserviceurl] --username [username] --password [password]" + "--vmname [VM name] --host [Name of Host] --localPath [path of local System]");
	}
