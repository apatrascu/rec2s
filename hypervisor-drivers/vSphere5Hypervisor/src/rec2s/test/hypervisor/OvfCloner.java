...
public class OvfCloner
{
	...
	private class HttpNfcLeaseExtender implements Runnable
	{
		...
		public void run()
		{
			try
			{
				while (!vmdkFlag)
				{
					if (LOG_ENABLED) { System.out.println("\n\n#####################vmdk flag: " + vmdkFlag + "\n\n"); }
					...
					{
						if (LOG_ENABLED) { System.out.println("********************** Thread interrupted *******************"); }
					} 
	...
	private static List<ObjectContent> retrievePropertiesAllObjects(List<PropertyFilterSpec> listpfs) throws Exception
	{
		...
		{
			if (LOG_ENABLED) { System.out.println(" : Failed Getting Contents"); }
			e.printStackTrace();
		}
	...
	private static void importVApp() throws Exception
	{
		try
		{
			...
			if (hostMor == null)
			{
				if (LOG_ENABLED) { System.out.println("Host System " + host + " Not Found."); }
				return;
			}
			...
			if (netWork != null && !netWork.isEmpty())
			{
				...
				if (!found)
				{
					if (LOG_ENABLED) { System.out.println("No virtual machine network found."); }
					return;
				}
			} else
			{
				if (LOG_ENABLED) { System.out.println("No network found."); }
				return;
			}
			...
			{
				if (LOG_ENABLED) { System.out.println("No datastore found."); }
				return;
			}
			...
			{
				if (LOG_ENABLED) { System.out.println("Datacenter Not Found."); }
				return;
			}
			...
			if (crArr != null && crArr.size() > 0)
			{
				...
				if (!found)
				{
					if (LOG_ENABLED) { System.out.println("Resource pool not found."); }
					return;
				}
			} 
			else
			{
				if (LOG_ENABLED) { System.out.println("ComputeResource Not Found."); }
				return;
			}
			...
			if (LOG_ENABLED) { System.out.println("Total bytes: " + TOTAL_BYTES); }
			if (ovfImportResult != null)
			{
				...
				{
					if (LOG_ENABLED) { System.out.println("HttpNfcLeaseState: " + result[0]); }
					...
					for (HttpNfcLeaseDeviceUrl deviceUrl : deviceUrlArr)
					{
						...
						for (OvfFileItem ovfFileItem : fileItemArr)
						{
							...
							{
								if (LOG_ENABLED) { System.out.println("Import key: " + deviceKey); }
								if (LOG_ENABLED) { System.out.println("OvfFileItem device id: " + ovfFileItem.getDeviceId()); }
								if (LOG_ENABLED) { System.out.println("HTTP Post file: " + ovfFileItem.getPath()); }
								String absoluteFile = localPath.substring(0, localPath.lastIndexOf("/"));
								absoluteFile = absoluteFile + "/" + ovfFileItem.getPath();
								if (LOG_ENABLED) { System.out.println("Absolute path: " + absoluteFile); }
								//getVMDKFile(ovfFileItem.isCreate(), absoluteFile, deviceUrl.getUrl().replace("*", host), ovfFileItem.getSize());
								getVMDKFile(ovfFileItem.isCreate(), absoluteFile, deviceUrl.getUrl().replace("*", clearIp), ovfFileItem.getSize());

								if (LOG_ENABLED) { System.out.println("Completed uploading the VMDK file"); }
							}
						}
					}
					vmdkFlag = true;
				...
				{
					if (LOG_ENABLED) { System.out.println("HttpNfcLeaseState not ready"); }
					for (Object o : result)
					{
						if (LOG_ENABLED) { System.out.println("HttpNfcLeaseState: " + o); }
					}
				}
			} 
			else
			{
				if (LOG_ENABLED) { System.out.println("--------------------OvfImportSpecResult is null---------------------"); }
			}
		}
		catch (SOAPFaultException sfe)
		{
	...
	private static void getVMDKFile(boolean put, String fileName, String uri, long diskCapacity)
	{
		...
		try
		{
			if (LOG_ENABLED) { System.out.println("Destination host URL: " + uri); }
			HostnameVerifier hv = new HostnameVerifier()
			{
				public boolean verify(String urlHostName, SSLSession session)
				{
					if (LOG_ENABLED) { System.out.println("Warning: URL Host: " + urlHostName + " vs. " + session.getPeerHost()); }
					return true;
				}
			};
			HttpsURLConnection.setDefaultHostnameVerifier(hv);
			...
			if (put)
			{
				...
				if (LOG_ENABLED) { System.out.println("HTTP method: PUT"); }
			} else
			{
				conn.setRequestMethod("POST");
				if (LOG_ENABLED) { System.out.println("HTTP method: POST"); }
			}
			...
			if (LOG_ENABLED) { System.out.println("Local file path: " + fileName); }
			...
			if (LOG_ENABLED) { System.out.println("vmdk available bytes: " + bytesAvailable); }
			...
			while (bytesRead >= 0)
			{
				...
				bos.flush();
				// if (LOG_ENABLED) { System.out.println("Bytes Wrote: " + bytesWrote);
				bytesAvailable = bis.available();
				bufferSize = Math.min(bytesAvailable, maxBufferSize);
				bytesWrote += bufferSize;
				// if (LOG_ENABLED) { System.out.println("Total bytes written: " +
				// TOTAL_BYTES_WRITTEN);
				TOTAL_BYTES_WRITTEN += bufferSize;
				buffer = null;
				buffer = new byte[bufferSize];
				bytesRead = bis.read(buffer, 0, bufferSize);
				// if (LOG_ENABLED) { System.out.println("Bytes Read: " + bytesRead);
				...
				if (LOG_ENABLED) { System.out.println("Progress: " + (TOTAL_BYTES_WRITTEN * 100.00 / fileSize)); }
			}
			try
			{
			...
			{
				if (LOG_ENABLED) { System.out.println("From (ServerResponse): " + stex); }
			} 
			catch (IOException ioex)
			{
				if (LOG_ENABLED) { System.out.println("From (ServerResponse): " + ioex); }
			}
			if (LOG_ENABLED) { System.out.println("Writing vmdk to the output stream done"); }
			bis.close();
	...
	private static String getOvfDescriptorFromLocal(String ovfDescriptorUrl) throws IOException
	{
		...
		{
			if (LOG_ENABLED) { System.out.println("Invalid local file path"); }
		}
	...
	{
		if (LOG_ENABLED) { System.out.println("##########################################################"); }
		if (LOG_ENABLED) { System.out.println("OvfFileItem"); }
		if (LOG_ENABLED) { System.out.println("chunkSize: " + fi.getChunkSize()); }
		if (LOG_ENABLED) { System.out.println("create: " + fi.isCreate()); }
		if (LOG_ENABLED) { System.out.println("deviceId: " + fi.getDeviceId()); }
		if (LOG_ENABLED) { System.out.println("path: " + fi.getPath()); }
		if (LOG_ENABLED) { System.out.println("size: " + fi.getSize()); }
		if (LOG_ENABLED) { System.out.println("##########################################################"); }
	}
	...
	{
		if (LOG_ENABLED) { System.out.println("########################################################"); }
		if (LOG_ENABLED) { System.out.println("HttpNfcLeaseInfo"); }
		// if (LOG_ENABLED) { System.out.println("cookie: " + info.getCookie());
		List<HttpNfcLeaseDeviceUrl> deviceUrlArr = info.getDeviceUrl();
		for (HttpNfcLeaseDeviceUrl durl : deviceUrlArr)
		{
			if (LOG_ENABLED) { System.out.println("Device URL Import Key: " + durl.getImportKey()); }
			if (LOG_ENABLED) { System.out.println("Device URL Key: " + durl.getKey()); }
			if (LOG_ENABLED) { System.out.println("Device URL : " + durl.getUrl()); }
			if (LOG_ENABLED) { System.out.println("Updated device URL: " + durl.getUrl().replace("*", "10.20.140.58")); }
		}
		if (LOG_ENABLED) { System.out.println("Lease Timeout: " + info.getLeaseTimeout()); }
		if (LOG_ENABLED) { System.out.println("Total Disk capacity: " + info.getTotalDiskCapacityInKB()); }
		if (LOG_ENABLED) { System.out.println("########################################################"); }
	}
	...
	{
		if (LOG_ENABLED) { System.out.println("This class can be used import or deploy an OVF Appliance from"); }
		if (LOG_ENABLED) { System.out.println("the Local drive."); }
		if (LOG_ENABLED) { System.out.println("\nParameters:"); }
		if (LOG_ENABLED) { System.out.println("host      [required] Name of the host system"); }
		if (LOG_ENABLED) { System.out.println("localpath [required] OVFFile LocalPath"); }
		if (LOG_ENABLED) { System.out.println("vappname  [required] New vApp Name"); }
		if (LOG_ENABLED) { System.out.println("\ncommand:"); }
		if (LOG_ENABLED) { System.out.println("run.bat com.vmware.samples.vapp.OVFManagerImportLocalVApp"); }
		if (LOG_ENABLED) { System.out.println("--url [webserviceurl] --username [username] --password"); }
		if (LOG_ENABLED) { System.out.println("[password] --host [hostname] --localpath [OVFFile LocalPath]"); }
		if (LOG_ENABLED) { System.out.println(" --vappname [New vApp Name]"); }
	}

	private static void printSoapFaultException(SOAPFaultException sfe)
	{
		if (LOG_ENABLED) { System.out.println("SOAP Fault -"); }
		if (sfe.getFault().hasDetail())
		{
			if (LOG_ENABLED) { System.out.println(sfe.getFault().getDetail().getFirstChild().getLocalName()); } 
		}
	...
	public static void clone(String[] args)
	{
		...
		{
			if (LOG_ENABLED) { System.out.println(iae.getMessage()); }
			printUsage();
		} catch (SOAPFaultException sfe)
		{
			printSoapFaultException(sfe);
		...
		{
			if (LOG_ENABLED) { System.out.println(" Failed : " + e); }
			e.printStackTrace();
		} finally
		{
			try
